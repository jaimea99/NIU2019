...
for i in range(entries): #eventually will loop over entries
        ...
        if abs(tree.jet_eta[j]) <= 2.5:
            if (((tree.jet_pt[j] > 40) and (tree.jet_pt[j] > highestjetpt))):
                    ...
                    try:#corrects for the absence of jet_label in data
                        ...
                        jetSF1 = tree.jet_SF_MV2c10_FixedCutBEff_70[j]
                    except:
                        continue
                    ...
                    try:
                        ...
                        jetSF2 = tree.jet_SF_MV2c10_FixedCutBEff_70[j]
                    except:
                        continue
    ...
    if ((two_photon_pass == True) and (two_bjet_pass == True)): #Set weights & Fill histograms
        weight = None
        jetweight = None
        if sys.argv[3] == '2tag' or sys.argv[3] == '0tag':
            jetweight = tree.yybb_low_weight
        elif sys.argv[3] == '1tag':
            if jetSF1 is not None and jetSF2 is not None:
                jetweight = mySF * jetSF1 * jetSF2
        ...
        if sys.argv[1] == 'data15.root' or sys.argv[1] == 'data16.root' or sys.argv[1] == 'h021/data1516.root' or sys.argv[1] == 'h021/data17.root':
            ...
            weight = tree.hgam_weight * jetweight * 0.005338
        elif sys.argv[1] == 'h021/Sherpa2_myy_90_175_mc16a.root':
            try:
                if sys.argv[4] == 'combined':
                    weight = tree.hgam_weight * jetweight * 0.0178421#data15-17
            except IndexError:#only use for data1516.root
                weight = tree.hgam_weight * jetweight * 0.01786#data1516
        elif sys.argv[1] == 'h021/Sherpa2_myy_90_175_mc16d.root':
            weight = tree.hgam_weight * jetweight * 0.017264#data15-17
                #try:
                    #if sys.argv[3] == 'combined':
                      #weight = 0.0094757
                #exept IndexEr      ror: #not used
                #   w   eight = 0.01728567
        if sys.argv[1] == 'data15.root' or sys.argv[1] == 'data16.root' or sys.argv[1] == 'h021/data1516.root' or sys.argv[1] == 'h021/data17.root':
            label = 'data'
